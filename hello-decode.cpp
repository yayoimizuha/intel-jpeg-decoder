...
void Usage(void) {
    ...
    printf("   Example:  hello-decode -i in.h265\n");
    printf("   To view:  ffplay -f rawvideo -pixel_format yuv420p -video_size "
           "[width]x[height] %s\n\n",
           OUTPUT_FILE);
...
int main(int argc, char *argv[]) {
    ...
    if (!ParseArgsAndValidate(argc, argv, &cliParams, PARAMS_DECODE)) {
        Usage();
    ...
    // Initialize session
    loader = MFXLoad();
    VERIFY(nullptr != loader, "MFXLoad failed -- is implementation in path?");

    // Implementation used must be the type requested from command line
    cfg[0] = MFXCreateConfig(loader);
    VERIFY(nullptr != cfg[0], "MFXCreateConfig failed")
    ...
    VERIFY(nullptr != cfg[1], "MFXCreateConfig failed")
    ...
    VERIFY(nullptr != cfg[2], "MFXCreateConfig failed")
